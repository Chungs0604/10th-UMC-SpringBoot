...
public class AuthController {
    ...
    @PostMapping("/signup")
    public ApiResponse<UserResDto.SignUpResponseDto> signup(@RequestBody @Valid UserReqDto.SignUpRequestDto dto) {

        UserResDto.SignUpResponseDto result = userService.createUser(dto);
        return ApiResponse.onSuccess(UserSuccessCode.SIGNUP_OK, result);
    }

    @PostMapping("/login")
    public ApiResponse<UserResDto.LoginResultDto> login(
            @RequestBody @Valid UserReqDto.loginRequestDto request) {
        return ApiResponse.onSuccess(UserSuccessCode.OK, userService.login(request));
    }
}
